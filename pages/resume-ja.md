...
フルリモートワークでのモバイルアプリ開発、メタバースプラットフォームのサーバーサイド開発を経て、現在はさくらインターネットにてソフトウェアエンジニア/チームリードに従事。
...
- 曖昧な要件を具体化し、組織横断で意思決定と実装を推進
- 開発プロセス（リリースフロー・品質管理・運用）の設計と改善
- テクニカルライティング/技術文書の作成
...
- 役割: ソフトウェアエンジニア、チームリード
- 利用技術: Go, PostgreSQL, Redis, Protocol Buffers, gRPC, Docker, Ansible, Terraform, GitHub Actions

ガバメントクラウドの技術要件を満たすための新規サービス開発において、要件定義から設計・実装・リリースまでを主導。   
チームリードとして実装だけでなく、プロダクト戦略・ロードマップ・PRD作成に必要な意思決定構造の整理も担い、PdMやBizと連携しながら優先順位付けと合意形成を推進。

- 曖昧な状態から発見的統制機能の要件を整理し、関係者とのコミュニケーションを通じて仕様の確度を向上
- プロダクト戦略、ロードマップ、PRDの関係を整理し、施策の優先順位を議論するためのドキュメント構成と意思決定フローを設計
- 運用高度化、有償機能開発、UX改善などの方針を、顧客価値・課題・候補機能の粒度に分解し、PdM/Bizとの合意形成を支援
- タイトなスケジュールの中で開発をリードし、期日内でのリリースを達成
- 可観測性・信頼性向上のための仕組みを導入し、品質と運用性を両立
- デイリーリリースの仕組みを導入し、リリースの属人性を排除しつつ開発スピードを向上
- 社内ツールの認証方式改善など、組織横断の技術課題を解決
- ドキュメント整備や開発フロー設計を通じて、チームの再現性ある開発体制を構築
- 技術実装に留まらず、チーム運営・開発プロセス・運用設計まで含めたプロダクト全体の構造設計を主導
...
また、単発の改善に留まらず、開発プロセスや設計指針を仕組みとして定着させることを重視している。直近のプロジェクトでは、リリースフローやドキュメント整備を通じて、属人性を排除しながらチーム全体の生産性と再現性を向上させた。

個人のアウトプット最大化だけでなく、チームや組織全体の能力を引き上げることを重視しており、他チームにも適用可能な形で知見や仕組みを展開している。
